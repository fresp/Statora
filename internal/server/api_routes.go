...
func RegisterAPIRoutes(r *gin.Engine, hub *handlers.Hub, cfg *configs.Config) {
	...
	api.GET("/status/availability", handlers.GetAvailability(database.GetDB()))
	...
	api.GET("/subscribe/verify", handlers.VerifySubscriber(database.GetDB()))
	api.GET("/subscribe/unsubscribe", handlers.Unsubscribe(database.GetDB()))
	api.POST("/subscribe", handlers.Subscribe(database.GetDB(), cfg))
	...
	incidentAndMaintenance.POST("/incidents", handlers.CreateIncident(database.GetDB(), hub, cfg))
	incidentAndMaintenance.GET("/incidents/:id", handlers.GetIncidentByID(database.GetDB()))
	incidentAndMaintenance.PATCH("/incidents/:id", handlers.UpdateIncident(database.GetDB(), hub, cfg))
	incidentAndMaintenance.DELETE("/incidents/:id", handlers.DeleteIncident(database.GetDB(), hub))
	incidentAndMaintenance.POST("/incidents/:id/resolve", handlers.ResolveIncident(database.GetDB(), hub, cfg))
	...
	incidentAndMaintenance.POST("/maintenance", handlers.CreateMaintenance(database.GetDB(), cfg))
	incidentAndMaintenance.GET("/maintenance/:id", handlers.GetMaintenanceByID(database.GetDB()))
	incidentAndMaintenance.PATCH("/maintenance/:id", handlers.UpdateMaintenance(database.GetDB(), cfg))
	...
	adminOnly.POST("/subscribers/:id/resend-verification", handlers.ResendVerification(database.GetDB(), cfg))
	adminOnly.GET("/settings/status-page", handlers.GetAdminStatusPageSettings(database.GetDB()))
	adminOnly.PATCH("/settings/status-page", handlers.UpdateStatusPageSettings(database.GetDB(), hub, cfg))
	adminOnly.POST("/settings/mail/test", handlers.SendTestEmail(database.GetDB(), cfg))
